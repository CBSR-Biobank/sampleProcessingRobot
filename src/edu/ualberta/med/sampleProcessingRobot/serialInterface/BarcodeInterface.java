...
public class BarcodeInterface {
	...
	static final byte[] BARCODE_START_READ = {
		0x31, 0x42, 0x30, 0x32, 0x35, 0x33, 0x30, 0x44, 0x30, 0x41 };
	 ...
	 * 
	 * @param portName Name of the COM port to connect to the Barcode reader on
	 */
	public BarcodeInterface(String portName) {
		port = new SerialInterface(portName,
				9600,
		...
		final class ReadCode extends Thread {
			...
			public void run() {
				...
				if(tempResponse.length() > 0)
					...
					response = tempResponse.substring(1, tempResponse.length() - 1);
			}
		}
